...
std::ostream& operator<<(std::ostream& os, Auteur auteur) {
    os << " Le nom de l'auteur est : " << auteur.getNom() << ". Le prénom de l'auteur est : " << auteur.getPrenom() << " L'id de l'auteur : " << auteur.getIdn() << " La date de l'auteur " << toString(auteur.getBorn()) << " La date de publication : " <<  std::endl;
    return os;
}