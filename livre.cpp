...
std::ostream& operator<<(std::ostream& os, Livre livre) {
    os << " Le titre du livre est : " << livre.getTitre() << ". L'auteur est : " << toString(livre.getAuteur()) << " La langue est : " << livre.getLangue() << " Le genre est :" << livre.getGenre() << " La date de publication : " << toString(livre.getDate_publi()) << std::endl ;
    return os;
}
